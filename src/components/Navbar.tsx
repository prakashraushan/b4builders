...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-[var(--background)]/95 backdrop-blur-md">
      ...
      >
        {/* Logo */}
        <Link href="/" aria-label="B4 Builders — home" className="flex shrink-0 items-center gap-2.5">
          <Image
            ...
            priority
          />
          <span className="font-[var(--font-fraunces)] text-xl font-medium tracking-tight text-[var(--text)]">
            B4 Builders
          </span>
        </Link>
        ...
        <div className="hidden items-center gap-7 lg:flex">
          ...
          >
            ...
            {buildOpen && (
              // Wrapper adds the visual gap as padding (not margin) so the cursor never leaves
              // the hoverable area while moving from the button down into the menu.
              <div className="absolute left-0 top-full z-50 w-56 pt-2">
                <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] py-1 shadow-lg">
                  {buildLinks.map((link) => (
                    <Link
                      key={link.href}
                      href={link.href}
